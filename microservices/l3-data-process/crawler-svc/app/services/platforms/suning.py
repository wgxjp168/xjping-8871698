...
_API_BASE = "https://open.suning.com/api/http/sopRequest"


class SuningAdapter(BasePlatformAdapter):
    platform = Platform.SUNING

    def _sign(self, params: Dict[str, str]) -> str:
        secret = settings.suning_app_key or "MOCK_SECRET"
    ...
    def _base_params(self, method: str) -> Dict[str, str]:
        return {
            "method":    method,
            "appKey":    settings.suning_app_key or "MOCK_KEY",
    ...
    async def search(
        ...
        params = self._base_params("suning.union.item.search")
        params.update({
            "keyword":  keyword,
        ...
        try:
            ...
            logger.error('"Suning search error keyword=%s: %s"', keyword, exc)
            if proxy_url:
        ...
        items = data.get("returnValue", {}).get("data", {}).get("itemInfoList", [])
    ...
    def _parse(self, item: Dict[str, Any],
        ...
        p = RawProduct(
            ...
            product_id     = str(item.get("commodityCode", "")),
            title          = item.get("commodityName", ""),
            price          = self._safe_float(item.get("salePrice", 0)),
            original_price = self._safe_float(item.get("orgPrice", 0)) or None,
            brand          = item.get("brandName", ""),
            category       = item.get("catgName", ""),
            images         = [item.get("picUrl", "")],
            specs          = {},
            sales_count    = self._safe_int(item.get("saleNum", 0)),
            review_count   = self._safe_int(item.get("commentNum", 0)),
            average_rating = self._safe_float(item.get("score", 0)) / 2.0,  # 0-10 → 0-5
            shop_name      = item.get("shopName", "苏宁易购"),
            in_stock       = item.get("inventoryStatus") != "SOLD_OUT",
            promotion      = bool(item.get("promotionInfo")),
    ...
    def _mock_products(self, keyword: str, n: int, session_id, job_id) -> List[RawProduct]:
        ...
        for i in range(min(n, 5)):
            p = RawProduct(
                platform       = self.platform,
                product_id     = f"suning_mock_{keyword}_{i}",
                title          = f"苏宁-{keyword}-商品{i+1}（模拟数据）",
                price          = round(129.9 + i * 80, 2),
                original_price = round(299.9 + i * 100, 2),
                brand          = f"品牌{i+1}",
                category       = "家电",
                specs          = {"颜色": "银色", "功率": "1500W"},
                sales_count    = 600 + i * 100,
                review_count   = 150 + i * 30,
                average_rating = round(4.1 + i * 0.1, 1),
                shop_name      = "苏宁自营",
                in_stock       = True,
                promotion      = i % 2 == 0,
                raw_data       = {"mock": True},
